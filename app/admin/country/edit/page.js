  ...
  useEffect(() => {
    const fetchData = async () => {
        try {
            ...
            const { data } = await axiosClient.get(`country/${id}`);
            console.log(data);
            setCountryName(data.result.name);
            setAlpha2(data.result.alpha2);
            setAlpha3(data.result.alpha3);
            setCountryCode(data.result.country_code);
            setIsoPort(data.result.iso_port);
            setIso3166(data.result.iso3166);
