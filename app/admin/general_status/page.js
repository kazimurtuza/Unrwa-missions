...
function GeneralStatus() {
    const [general_status, setGeneral_Status] = useState([]);
    const api_base_url = process.env.NEXT_PUBLIC_API_BASE_URL;

    const fetchData = async () => {
        try {
            const { data } = await axiosClient.get('general_status');
            setGeneral_Status(data.result);
    ...
    let tableName = "Mission General Status";
    const headName = ["Status", "Action"];
    let head = (
        <tr>
    ...
    const body = (
        <>
            {Array.isArray(general_status) && general_status.map((item, index) => (
