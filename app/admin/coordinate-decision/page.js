...
function CoordinationDecision() {
    const [coordinationDecision, setCoordinationDecision] = useState([]);
    const api_base_url = process.env.NEXT_PUBLIC_API_BASE_URL;

    const fetchData = async () => {
        try {
            const { data } = await axiosClient.get('coordination_decision');
            setCoordinationDecision(data.result);
    ...
    let tableName = "Missions CLA Movement Coordination Decision";
    const headName = ["Decision", "Action"];
    let head = (
        <tr>
    ...
    const body = (
        <>
            {Array.isArray(coordinationDecision) && coordinationDecision.map((item, index) => (
...
export default CoordinationDecision;
