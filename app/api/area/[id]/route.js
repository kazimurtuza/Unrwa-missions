...
export async function PUT(request, content) {
    ...
    try {
        ...
        const missionCluster=await Area.findById(filter);
        const oldData=missionCluster._doc;
        const record = { name: payload.name, is_delete: 0 };
        const is_findData = await Area.findOne({
            ...record,
...
export async function GET(request, content) {
    ...
    try {
        ...
        const record = {_id: id};
        await mongoose.connect(connectionStr);
        const result = await Area.findById(record);
        return NextResponse.json({result, success: true});
    } catch (error) {
        result = error;
    }
...
export async function DELETE(request, content) {
    try {
        ...
        await mongoose.connect(connectionStr);
        const mission = await Area.findById(filter);
