...
export async function GET(){
    ...
    try{
        ...
        data = await CoordinationDecision
...
export async function POST(request) {
    try {
        ...
        const is_findData = await CoordinationDecision.findOne(record);
        if (is_findData) {
            return NextResponse.json({msg: 'Decision must be unique',success:false}, {status: 409});
        }

        //create
        let department = new CoordinationDecision(payload);
